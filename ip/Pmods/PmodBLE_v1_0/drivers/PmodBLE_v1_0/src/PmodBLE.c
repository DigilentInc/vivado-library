...
/*	Pmod BLE using simple reads and writes. 							*/
/*																		*/
/************************************************************************/
/*  Revision History:													*/
/*																		*/
/*	01/08/2018(TommyK): Created 										*/
/*																		*/
/************************************************************************/
...
void BLE_begin(PmodBLE* InstancePtr, u32 GPIO_Address, u32 UART_Address)
{
	...
	Xil_Out32(InstancePtr->GPIO_addr+4, 0b1001);//Set tristates
	BLE_UARTInit(&InstancePtr->BLEUart);
}

/* ------------------------------------------------------------ */
/*  BLE_getData(PmodBLE* InstancePtr, int buffersize)
**
**  Parameters:
...
int BLE_getData(PmodBLE* InstancePtr, int buffersize)
{
	...
	ReceivedCount += XUartNs550_Recv(&InstancePtr->BLEUart, InstancePtr->recv, buffersize);

	return ReceivedCount;
}

/* ------------------------------------------------------------ */
/*  BLE_sendData(PmodBLE* InstancePtr, char* sendData, int size)
**
**  Parameters:
**	  InstancePtr:	PmodBLE object to configure
...
int BLE_sendData(PmodBLE* InstancePtr, char* sendData, int size)
{
	...
	unsigned int ReceivedCount = 0;
	while(i<size){
		i+= XUartNs550_Send(&InstancePtr->BLEUart, sendData+i, size-i);
	}
...
int BLE_changeBaud(PmodBLE* InstancePtr, int baud){
	XUartNs550_SetBaud(InstancePtr->BLEUart.BaseAddress, PERIPHERAL_CLK,baud);

	return XST_SUCCESS;
}

/* ------------------------------------------------------------ */
/*	BLE_UARTInit(XUartNs550 *UartInstancePtr)
**
**	Parameters:
...
**
**	Description:
**		Initializes the PmodBLE UART.
**
******************************************************************/
int BLE_UARTInit(XUartNs550 *UartInstancePtr){
	int Status;
	u16 Options;
	Status = XUartNs550_CfgInitialize(UartInstancePtr, &BLE_Config, BLE_Config.BaseAddress);
...
/* ------------------------------------------------------------ */
/*  BLE_ReadRTS(PmodBLE* InstancePtr)
**
**  Parameters:
...
/* ------------------------------------------------------------ */
/*  BLE_WriteCTS(PmodBLE* InstancePtr, bool enable)
**
**  Parameters:
**	  InstancePtr:	PmodBLE object to read from
**	  bool:			Enable
...
/* ------------------------------------------------------------ */
/*  BLE_getLine(PmodBLE* InstancePtr, char* output)
**
**  Parameters:
**	  InstancePtr:	PmodBLE object to read from
...
bool BLE_getLine(PmodBLE* InstancePtr, char* output){
	int ReceivedCount=0;
	if (ReceivedCount += XUartNs550_Recv(&InstancePtr->BLEUart, (u8*)output+ReceivedCount, 1)){	//If there is a sentence
		while(output[ReceivedCount-1] != '\n'){
			ReceivedCount += XUartNs550_Recv(&InstancePtr->BLEUart, (u8*)output+ReceivedCount, 1);
		}
		return 1;
	}
	else return 0;
...
/* ------------------------------------------------------------ */
/*  BLE_SetupInterruptSystem(PmodBLE* InstancePtr, u32 interruptDeviceID, u32 interruptID, void* recvHandler, void* sendHandler, void* timeoutHandler)
**
**  Parameters:
**	  InstancePtr:			PmodBLE object to get data from
**	  interruptDeviceID:	Device ID of the interrupt controller
**	  interruptID:			The vector ID of the BLE UART interrupt
**
**  Return Value:
...
int BLE_SetupInterruptSystem(PmodBLE* InstancePtr, u32 interruptDeviceID, u32 interruptID, void* recvHandler, void* sendHandler, void* timeoutHandler)
{
	...
	XIntc_Connect(IntcInstancePtr, interruptID,
		      (Xil_ExceptionHandler)XUartNs550_InterruptHandler, &InstancePtr->BLEUart);
	...
	XUartNs550_SetHandler(&InstancePtr->BLEUart, (void*)BLE_intHandler, InstancePtr);
	...
	XUartNs550_SetOptions(&InstancePtr->BLEUart, Options);

#endif
#ifdef XPAR_FABRIC_PMODBLE_0_BLE_UART_INTERRUPT_INTR
	INTC *IntcInstancePtr = &InstancePtr->intc;
...
/* ------------------------------------------------------------ */
/*  BLE_intHandler(PmodBLE* InstancePtr, u32 Event, unsigned int EventData)
**
**  Parameters:
**	  InstancePtr:	PmodBLE object to get data from
