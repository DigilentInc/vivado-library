...
/* ------------------------------------------------------------ */
/***	void CMPS2_begin(PmodCMPS2* InstancePtr, u32 IIC_Address)
**
**	Parameters:
...
void CMPS2_begin(PmodCMPS2* InstancePtr, u32 IIC_Address, u8 Chip_Address)
{
	...
	XIic_SetAddress(&InstancePtr->CMPS2Iic, XII_ADDR_TO_SEND_TYPE, InstancePtr->chipAddr);
}
...
int CMPS2_IICInit(XIic *IicInstancePtr)
{
	...
	Status = XIic_CfgInitialize(IicInstancePtr, &CMPS2_Config, CMPS2_Config.BaseAddress);
...
void CMPS2_ReadIIC(PmodCMPS2* InstancePtr, u8 reg, u8 *Data, int nData)
{
	...
	if (InstancePtr->currentRegister != reg){
		XIic_Send(InstancePtr->CMPS2Iic.BaseAddress, InstancePtr->chipAddr, &reg, 1, XII_REPEATED_START_OPTION);
		InstancePtr->currentRegister = reg;
	}

	XIic_Recv(InstancePtr->CMPS2Iic.BaseAddress, InstancePtr->chipAddr, Data, nData, XIIC_STOP);
...
void CMPS2_WriteIIC(PmodCMPS2* InstancePtr, u8 reg, u8 *Data, int nData)
{
	...
	XIic_Send(InstancePtr->CMPS2Iic.BaseAddress, InstancePtr->chipAddr, out, nData+1, XIIC_STOP);
...
**
**	Description:
**		Selects one of 4 possible resolution options, see page 8 of Memsic MMC3416xPJ data sheet
**
*/
