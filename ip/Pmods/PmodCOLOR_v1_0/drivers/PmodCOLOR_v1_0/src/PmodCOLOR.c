...
void COLOR_Begin(PmodCOLOR* InstancePtr, u32 IicBaseAddr, u32 GpioBaseAddr, u8 IicChipAddr)
{
	...
	Status = XIic_CfgInitialize(&InstancePtr->Iic, &COLOR_IicConfig, COLOR_IicConfig.BaseAddress);
 ...
 **
 **     Description:
 **             Read data from the Pmod COLOR. Note that in order to read registers,
...
void COLOR_ReadIIC(PmodCOLOR* InstancePtr, u8 reg, u8 *Data, int nData)
{
	...
	if (InstancePtr->CurrentReg != reg){
		XIic_Send(InstancePtr->Iic.BaseAddress, InstancePtr->ChipAddr, &reg, 1, XII_REPEATED_START_OPTION);
		InstancePtr->CurrentReg = reg;
	}
	XIic_Recv(InstancePtr->Iic.BaseAddress, InstancePtr->ChipAddr, Data, nData, XIIC_STOP);
...
void COLOR_WriteIIC(PmodCOLOR* InstancePtr, u8 reg, u8 *Data, int nData)
{
	...
	for (i=0; i<nData; i++) out[i+1] = Data[i];

	if (InstancePtr->CurrentReg != reg) InstancePtr->CurrentReg = reg;
	Status = XIic_Start(&InstancePtr->Iic);
	if (Status != XST_SUCCESS) return;

	XIic_Send(InstancePtr->Iic.BaseAddress, InstancePtr->ChipAddr, out, nData+1, XIIC_STOP);
 ...
 **
 **     Description:
 **             Reads the state of the Pmod COLOR interrupt GPIO
 */
u32 COLOR_GetINT(PmodCOLOR *InstancePtr) {
 ...
 **
 **     Description:
 **             Retrieves a data packet from the Pmod COLOR
 */
COLOR_Data COLOR_GetData(PmodCOLOR *InstancePtr) {
	...
	u8 buffer[8];
	XIic_Send(InstancePtr->Iic.BaseAddress, InstancePtr->ChipAddr, &reg, 1, XII_REPEATED_START_OPTION);
	XIic_Recv(InstancePtr->Iic.BaseAddress, InstancePtr->ChipAddr, buffer, 8, XIIC_STOP);
 ...
 **
 **     Return Value:
 **             The contents of the device ID register on the Pmod COLOR, 0x44 or 0x10
 **
 **     Errors:
 **             none
 **
 **     Description:
 **             Retrieves the Pmod COLOR's device ID
 */
u8 COLOR_GetID(PmodCOLOR *InstancePtr)
{
	...
	XIic_Send(InstancePtr->Iic.BaseAddress, InstancePtr->ChipAddr, &reg, 1, XII_REPEATED_START_OPTION);
	XIic_Recv(InstancePtr->Iic.BaseAddress, InstancePtr->ChipAddr, bytes, 1, XIIC_STOP);
	return bytes[0];
}
 ...
 **
 **     Description:
 **             Sets the contents of the Pmod COLOR's ENABLE register, used in the power up and initialization process
 */
void COLOR_SetENABLE(PmodCOLOR *InstancePtr, u8 bits)
{
	u8 bytes[2] = {
	...
	XIic_Send(InstancePtr->Iic.BaseAddress, InstancePtr->ChipAddr, bytes, 2, XIIC_STOP);
}
