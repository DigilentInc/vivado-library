...
void KYPD_begin(PmodKYPD* InstancePtr, u32 GPIO_Address)
{
    ...
    //set tri-state register, lower 4 pins are column outputs, upper 4 pins are row inputs
...
u16 KYPD_getKeyStates(PmodKYPD *InstancePtr)
{
	...
	// test each column combination, this will help to detect when
	// multiple keys in the same row are pressed.
...
void KYPD_loadKeyTable(PmodKYPD *InstancePtr, u8 keytable[16])
{
	...
	for (i=0; i<16; i++)
	{
		InstancePtr->keytable[i] = keytable[i]; // hard copy string into device struct
	}
...
**  Parameters:
**    InstancePtr: A PmodKYPD device to use
**    keystate:    Status of each key, pressed indicated by a '1' in that key's bit position.
**    			   Bit positions correspond with character positions in InstancePtr->keytable.
**    cptr:        Address to return additional information when a single key is pressed.
**
**  Return Value:
**    status:
**    	XST_SUCCESS when only one key is pressed.
**    	  cptr is loaded with a human-readable character when keytable is loaded.
...
**
**  Errors:
**    KYPD_MULTI_KEY - cannot differentiate which keys are pressed in some multi-key cases.
**
**  Description:
**    Set the keytable array so that KYPD_getKeyPressed will return will
**      return the human-readable character that the pressed key represents
...
u32 KYPD_getKeyPressed(PmodKYPD *InstancePtr, u16 keystate, u8 *cptr)
{
	...
	if (count > 1) // Multiple keys pressed, cannot differentiate which.
	{
		return KYPD_MULTI_KEY;
	}
	else if (count == 0) // No key pressed.
	{
		return KYPD_NO_KEY;
	}
	else // One key pressed.
	{
